	...
	if(mHighestBandIndex >= 2)
	{
		for (int L = 2; L <= int(mHighestBandIndex); ++L)
		{
			...
			for (int M = 0; M <= L - 1; ++M)
			{
				...
				int symmetricM = L;
				int symmetricN = M;
				SHRotationWignerMatrix::SetByIndex(L, symmetricM, symmetricN, powf(-1, symmetricM - symmetricN) * scaleFactor* prevTerm1);
				SHRotationWignerMatrix::SetByIndex(L, -symmetricM, -symmetricN, powf(-1, symmetricN - symmetricM) * scaleFactor* prevTerm2);
			}
