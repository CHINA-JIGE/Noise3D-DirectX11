...
IOscilloscope::IOscilloscope(float maxAmplitude, float signalTimeInterval, UINT(*pFunc)(UINT NoiseGUIEvent))
{
	if(pFunc!=nullptr)m_pFunction_EventMessageProcess = pFunc;
	m_pGraphicObj_Grid = new IGraphicObject;
	m_pGraphicObj_Wave = new IGraphicObject;
...
void IOscilloscope::SetBackGroundTexture(UINT texID)
{
	mBackGroundTexID = texID;
}

void IOscilloscope::SetWaveGraphicType(BOOL isLineSegments)
{
	if (isLineSegments)
	{
		mWaveGraphicTopoType = NOISE_GRAPHIC_OBJECT_TYPE_LINE_2D;
...
void IOscilloscope::SetMaxAmplitude(float waveAmp)
{
	mMaxAmplitude = waveAmp > 0.0f ? waveAmp : 1.0f;
}

void IOscilloscope::SetGridColor(NVECTOR4 color)
{
	*m_pGridColor = color;
}

void IOscilloscope::SetWaveColor(NVECTOR4 color)
{
	*m_pGridColor = color;
}

void IOscilloscope::SetScreenTimeRange(float timeMin, float timeMax)
{
	mScreenTimeRange_Min =  timeMin;
	mScreenTimeRange_Max = timeMax;
}

void IOscilloscope::SetSignalTimeInterval(float time_ms)
{
	mSignalTimeInterval = time_ms > 0.0f ? time_ms : 0.0f;
}

void IOscilloscope::SetTriggerRecordingTime(float time_ms)
{
	//at least record one signal
	mTriggerRecordingTime = time_ms > mSignalTimeInterval ? time_ms : mSignalTimeInterval;
}

UINT IOscilloscope::GetBackGroundTexture()
{
	return mBackGroundTexID;
}

float IOscilloscope::GetMaxAmplitude()
{
	return mMaxAmplitude;
}

NVECTOR4 IOscilloscope::GetGridColor()
{
	return *m_pGridColor;
}

NVECTOR4 IOscilloscope::GetWaveColor()
{
	return *m_pWaveColor;
}

void IOscilloscope::PushSignal(float signalAmplitude)
{
	//--------------------------TRIGGER CONDITION-------------------------
...
void IOscilloscope::Destroy()
{
}

void  IOscilloscope::mFunction_Update(BOOL isMouseInContainer, N_GUI_MOUSE_INFO & mouseInfo)
{
	//validate 
...
void  IOscilloscope::mFunction_UpdateGraphicObject()
{
...
void IOscilloscope::mFunction_ResampleSignalToFitTheScreen(UINT sequenceID, std::vector<NVECTOR2>& outResampledSequence)
{
