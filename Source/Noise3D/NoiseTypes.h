...
namespace Noise3D
{
	...
	struct N_Ray
	{
		N_Ray():origin(NVECTOR3(0,0,0)), dir(NVECTOR3(0, 0, 0)), t_min(0.001f), t_max(std::numeric_limits<float>::infinity()){}
		N_Ray(NVECTOR3 _origin, NVECTOR3 _dir) :origin(_origin), dir(_dir), t_min(0.001f), t_max(std::numeric_limits<float>::infinity()) {}
		N_Ray(NVECTOR3 _origin,NVECTOR3 _dir, float _t_min, float _t_max) :origin(_origin),dir(_dir),t_max(_t_max), t_min(_t_min) {}
