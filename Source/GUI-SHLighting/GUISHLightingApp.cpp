...
{
	QMessageBox::information(this, tr(u8"¹ØÓÚ"), tr(u8"×÷Õß£ºsunhonglian"));
}
...
void GUISHLightingApp::Slot_LoadSphericalTexture()
{
	...
	fileDialog->setWindowTitle(tr(u8"´ò¿ªÍ¼Æ¬"));
	fileDialog->setDirectory(u8"./");//default directory
	fileDialog->setNameFilter(tr(u8"Í¼Ïñ(*.png *.jpg *.jpeg *.bmp *.tga *.gif)"));//file format filter
	...
	{
		QMessageBox::information(this, tr(u8"´íÎó"), tr(u8"ÇëÑ¡ÔñÒ»¸öÍ¼ÏñÎÄ¼þ£¡"));
	}
	else
	{
...
void GUISHLightingApp::Slot_LoadCubeMap()
{
	...
	fileDialog->setWindowTitle(tr(u8"´ò¿ªÍ¼Æ¬"));
	fileDialog->setDirectory(u8"./");//default directory
	fileDialog->setNameFilter(tr(u8"CubeMapÍ¼Æ¬¸ñÊ½(*.dds)"));//file format filter
	fileDialog->setFileMode(QFileDialog::FileMode::ExistingFile);//select what kind of object, file/files/dir??
	fileDialog->setViewMode(QFileDialog::ViewMode::Detail);
