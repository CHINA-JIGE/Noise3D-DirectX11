...
BOOL Init3D(HWND hwnd)
{
	...
	for (int rayId = 0; rayId < c_rayCount; ++rayId)
	{
		...
		Vec3 dir = { g.CanonicalReal() * (-1.5f*origin.x),g.CanonicalReal() * (-1.5f*origin.y),g.CanonicalReal() * (-1.5f*origin.z) };
	...
	pCT->RebuildBvhTree(sg);//re-build BVH tree
	for (int rayId = 0; rayId < c_rayCount; ++rayId)
	{
		N_Ray r = rayArray.at(rayId);
		N_RayHitResult hitRes;
		bool hit = pCT->IntersectRayScene(r, hitRes);
		if (hit)anyHit_List.at(rayId) = true;
