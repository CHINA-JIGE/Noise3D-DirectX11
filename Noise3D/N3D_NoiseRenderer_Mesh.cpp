...
#pragma region Render Mesh
	...
	{
		//取出渲染列表中的mesh指针
		NoiseMesh* const tmp_pMesh = m_pRenderList_Mesh->at(i);
...
void		NoiseRenderer::mFunction_RenderMeshInList_UpdateCbPerFrame(NoiseCamera*const pCamera)
{
	//————更新Dynamic Light————
	NoiseLightManager* tmpLightMgr = m_pFatherScene->m_pChildLightMgr;
	if (tmpLightMgr != NULL)
...
void		NoiseRenderer::mFunction_RenderMeshInList_UpdateCbPerSubset(NoiseMesh* const pMesh,UINT subsetID)
{
...
void		NoiseRenderer::mFunction_RenderMeshInList_UpdateCbPerObject(NoiseMesh* const pMesh)
{
	//————更新World Matrix————
