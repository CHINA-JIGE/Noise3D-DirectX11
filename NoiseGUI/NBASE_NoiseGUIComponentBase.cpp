...
NoiseGUIComponentBase::NoiseGUIComponentBase()
{
	mIsFocused = FALSE;
	mDepthLevel = 0;
...
void	NoiseGUIComponentBase::SetEventProcessCallbackFunction(UINT(*pFunc)(UINT NoiseGUIEvent))
{
	if (pFunc != nullptr)m_pFunction_EventMessageProcess = pFunc;
};

//active event acquisition
void	NoiseGUIComponentBase::GetEventList(std::vector<UINT>& outEventList)
{
...
//是否已获取焦点
BOOL NoiseGUIComponentBase::IsFocused() 
{
	return mIsFocused; 
}

void NoiseGUIComponentBase::SetDepthLevel(UINT depth)
{
...
UINT NoiseGUIComponentBase::GetDepthLevel()
{
	return mDepthLevel;
}
...
************************************************************************/
 ...
 void NoiseGUIComponentBase::mFunction_GenerateMouseEventsAndValidateFocus(std::vector<UINT>& outEventList, BOOL isMouseInContainer, N_GUI_MOUSE_INFO& mouseInfo)
{
	//1.validate focus
...
void	NoiseGUIComponentBase::mFunction_CallMsgProcFunctionAndUpdateEventList(std::vector<UINT>& eventList)
{
