...
Noise3D::NOISE_TEXTURE_SAVE_FORMAT GetFormat(std::string fm);
...
Noise3D::NOISE_TEXTURE_SAVE_FORMAT GetFormat(std::string fm)
{
	if (fm == "bmp")return NOISE_TEXTURE_SAVE_FORMAT_BMP;
	if (fm == "jpg")return NOISE_TEXTURE_SAVE_FORMAT_JPG;
	if (fm == "dds")return NOISE_TEXTURE_SAVE_FORMAT_DDS;
	std::cout << "Image format not supported!! Defaulted to .dds!" << std::endl;
	return NOISE_TEXTURE_SAVE_FORMAT_DDS;
}
